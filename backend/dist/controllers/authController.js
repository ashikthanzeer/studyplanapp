    ...
    try {
        ...
        try {
            ...
            for (let i = 0; i < defaultColumns.length; i++) {
                await dbClient.query('INSERT INTO kanban_columns (user_id, name, position) VALUES ($1, $2, $3)', [user.id, defaultColumns[i], i + 1]);
            }
...
async function login(req, res) {
    try {
        ...
        const result = await (0, connection_1.query)('SELECT id, email, password_hash FROM users WHERE email = $1', [email]);
...
async function getUserProfile(req, res) {
    try {
        ...
        const result = await (0, connection_1.query)(`SELECT u.id, u.email, sp.name, sp.avatar_url, sp.bio
       FROM users u
