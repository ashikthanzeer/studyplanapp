...
exports.updateStreakAndBadges = updateStreakAndBadges;
...
async function awardBadge(userId, badgeName) {
    ...
    const badgeRes = await (0, connection_1.query)(`SELECT id, count, DATE(last_earned_at)::text as last_earned_date 
     FROM user_badges 
     WHERE user_id = $1 AND badge_name = $2`, [userId, badgeName]);
    const dateRes = await (0, connection_1.query)(`SELECT CURRENT_DATE::text as today`);
    const today = dateRes.rows[0].today;
    if (badgeRes.rows.length === 0) {
        // Insert new badge if the user doesn't have it
        await (0, connection_1.query)(`INSERT INTO user_badges (user_id, badge_name, count, last_earned_at)
       VALUES ($1, $2, 1, CURRENT_TIMESTAMP)`, [userId, badgeName]);
        ...
        if (badge.last_earned_date !== today) {
            await (0, connection_1.query)(`UPDATE user_badges 
 ...
 */
async function updateStreakAndBadges(userId) {
    try {
        // Get server date information in DB timezone
        const dateRes = await (0, connection_1.query)(`SELECT CURRENT_DATE::text as today, (CURRENT_DATE - INTERVAL '1 day')::date::text as yesterday`);
        const today = dateRes.rows[0].today;
        const yesterday = dateRes.rows[0].yesterday;
        // 1. Evaluate User Streak
        const streakRes = await (0, connection_1.query)(`SELECT id, current_streak, max_streak, DATE(last_activity_date)::text as last_activity 
       FROM user_streaks 
       WHERE user_id = $1`, [userId]);
        ...
        if (streakRes.rows.length === 0) {
            // First streak record
            await (0, connection_1.query)(`INSERT INTO user_streaks (user_id, current_streak, max_streak, last_activity_date)
        ...
        const dailyMinutesRes = await (0, connection_1.query)(`SELECT SUM(duration_minutes) as daily_minutes 
       FROM pomodoro_sessions 
       WHERE user_id = $1 AND status = 'completed' AND DATE(ended_at) = CURRENT_DATE`, [userId]);
