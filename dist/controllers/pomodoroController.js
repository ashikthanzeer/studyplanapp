...
async function completePomodoroSession(req, res) {
    try {
        ...
        if (typeof duration_minutes === 'number') {
            result = await (0, connection_1.query)(`UPDATE pomodoro_sessions SET
          status = 'completed', ended_at = CURRENT_TIMESTAMP, duration_minutes = $1
         WHERE id = $2 AND user_id = $3
         RETURNING *`, [duration_minutes, id, req.user.id]);
        }
        else {
            result = await (0, connection_1.query)(`UPDATE pomodoro_sessions SET
...
async function getSessionStats(req, res) {
    ...
    `;
        ...
        (SELECT COALESCE(SUM(duration_minutes), 0) FROM pomodoro_sessions WHERE user_id = $1 AND status = 'completed' AND ended_at >= (DATE_TRUNC('week', CURRENT_DATE) - INTERVAL '1 week') AND ended_at < DATE_TRUNC('week', CURRENT_DATE)) as previous_week_minutes`, [req.user.id]);
