...
async function getGoals(req, res) {
    try {
        ...
        const result = await (0, connection_1.query)('SELECT * FROM study_goals WHERE user_id = $1 ORDER BY id ASC', [req.user.id]);
    ...
    try {
        ...
        try {
            ...
            for (const goal of goals) {
                if (goal.id) {
                    await dbClient.query('UPDATE study_goals SET target_hours = $1, title = COALESCE($2, title), period = COALESCE($3, period), updated_at = CURRENT_TIMESTAMP WHERE id = $4 AND user_id = $5', [goal.target_hours, goal.title, goal.period, goal.id, req.user.id]);
                }
                else {
                    await dbClient.query('INSERT INTO study_goals (user_id, title, target_hours, period) VALUES ($1, $2, $3, $4)', [req.user.id, goal.title || 'Target', goal.target_hours, goal.period || 'weekly']);
                }
            }
